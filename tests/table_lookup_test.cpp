...
TEST(plato_Temperature_solve_test)
{
    ...
    real_t T_inv_table = eos.biinterp_inverse_table(*phys, L, S1, thermoTables);
    real_t T_newton    = eos.temp_from_internal_energy(*phys, L, S1, thermoTables);

    real_t P_true = mix.P;
    real_t P_tab_direct  = eos.biinterp_lte_table(L.P_idx,*phys, L, S1, T, thermoTables);
    real_t P_tab_newton  = eos.pressure(*phys, L, S1, thermoTables);
    real_t e_newton = eos.biinterp_lte_table(L.e_idx,*phys, L, S1, T_newton, thermoTables);
    ...
    real_t rhoe_inverse = eos.internal_energy_from_pressure(*phys, L, S_random, P_true, thermoTables);
...
TEST(plato_Tablelookup_test)
{
    ...
    real_t P_table  = eos.pressure(*phys, L, S1, thermoTables);
    ...
    P_table  = eos.pressure(*phys, L, S2, thermoTables);
    ...
    P_table  = eos.pressure(*phys, L, S3, thermoTables);
    rel_err = std::abs(mix.P - P_table)/std::abs(mix.P);
    ...
    for(real_t rho_true = rho_grid[3]; rho_true <= rho_grid[4] ; rho_true += 0.0001)
    {
        ...
        real_t P_table = eos.pressure(*phys, L, S4, thermoTables);
        ...
        real_t rhoe_inverse = eos.internal_energy_from_pressure(*phys, L, S5, P_table, thermoTables);
