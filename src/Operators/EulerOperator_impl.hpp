...
namespace Prandtl
{
  ...
  void EulerOperator<PhysicsT>::Finalize(real_t time)
  {    
    this->SetTime(time);
  ...
  template<typename PhysicsT>
  EulerOperator<PhysicsT>::~EulerOperator()
  {}

#ifdef SUBCELL_FV_BLENDING  
  template<typename PhysicsT>
  void EulerOperator<PhysicsT>::ComputeIndicatorField(const mfem::Vector &u,
  ...
  template<typename PhysicsT>
  void EulerOperator<PhysicsT>::ComputeBlendingCoefficientFromIndicator(const Vector &indicator_field) const
  {
    {
  ...
  template<typename PhysicsT>
  void EulerOperator<PhysicsT>::ComputeIntegralMeasures(const Vector &u, IntegralMeasures &diag) const
  {
    ScopedTimer timer("ComputeIntegralMeasures");
  ...
  template<typename PhysicsT>
  void EulerOperator<PhysicsT>::Mult(const mfem::Vector &u, mfem::Vector &dudt) const
  {
    ScopedTimer timer("EulerRHS");
...
#ifdef SUBCELL_FV_BLENDING
    {
      ScopedTimer timer("SubcellBlendingStep");
  ...
  template<typename PhysicsT>
  real_t EulerOperator<PhysicsT>::MultEuler_Volume(const Vector &pu, Vector &pdudt) const
  {
  ...
  template<typename PhysicsT>
  real_t EulerOperator<PhysicsT>::MultEuler_InteriorFaces(const Vector &pu, Vector &pdudt) const
  {
  ...
  template<typename PhysicsT>
  real_t EulerOperator<PhysicsT>::MultEuler_BoundaryFaces(const Vector &pu, Vector &pdudt) const
  {
  ...
  template<typename PhysicsT>
  real_t EulerOperator<PhysicsT>::MultEuler(const Vector &u, Vector &dudt) const
  {
  ...
  template<typename PhysicsT>
  void EulerOperator<PhysicsT>::AddBdrFaceIntegrator(BdrFaceIntegrator *bfi, Array<int> &bdr_marker_)
  {
