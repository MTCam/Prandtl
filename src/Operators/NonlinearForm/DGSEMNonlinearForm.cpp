...
void DGSEMNonlinearForm::GradOperatorVolumeDevice(const Vector &pu, std::vector<mfem::Vector *> &p_grad_u) const
{
...
void DGSEMNonlinearForm::GradOperatorBoundaryFacesHost(const mfem::Vector &pu,
                                                       std::vector<mfem::Vector *> &p_grad_u) const
{
  ...
  if(dim == 2){
    Vector &pdudx(*p_grad_u[0]);
    Vector &pdudy(*p_grad_u[1]);
    if (bfnfi.Size())
      {
        FaceElementTransformations *tr;
        const FiniteElement *fe1, *fe2;
...
void DGSEMNonlinearForm::GradOperatorInteriorFacesHost(const mfem::Vector &pu,
                                                       std::vector<mfem::Vector *> &p_grad_u) const
{
  const int dim = cache->dim;
            ...
            for (int i = 0; i < n_shared_faces; i++)
              {
                ...
                for (int k = 0; k < fnfi.Size(); k++)
                  {
                    fnfi[k]->AssembleLiftingFaceVector(*fe1, *fe2, *tr, el_u, el_dudx, el_dudy);
                    pdudx.AddElementVector(vdofs1, el_dudx.GetData());
                    pdudx.AddElementVector(vdofs1, el_dudy.GetData());
                  }
              }
          }
      }
  ...
  void DGSEMNonlinearForm::GradOperatorBoundaryFacesDevice(const mfem::Vector &pu,
                                                           std::vector<mfem::Vector *> &p_grad_u) const
  {
    const int dim = cache->dim;
  ...
  void DGSEMNonlinearForm::GradOperatorVolumeHost(const mfem::Vector &pu,
                                                  std::vector<mfem::Vector *> &p_grad_u) const
  {
    const int dim = cache->dim;
    if(dim == 2){
...
void DGSEMNonlinearForm::GradOperatorInteriorFacesDevice(const Vector &pu,
                                                         std::vector<mfem::Vector *> &p_grad_u) const
{
  const int dim = cache->dim;
  MFEM_ASSERT(dim == 2, "Only dim=2 supported right now.");
                                      ...
                                      std::vector<mfem::Vector *> &grad_u) const
{
  ...
  GradOperatorVolumeDevice(pu, p_grad_u);
 
  GradOperatorInteriorFacesDevice(pu, p_grad_u);

  GradOperatorBoundaryFacesDevice(pu, p_grad_u);
