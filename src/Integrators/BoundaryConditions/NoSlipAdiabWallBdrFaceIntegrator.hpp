...
public:
  NoSlipAdiabWallBdrFaceIntegrator(std::shared_ptr<LiftingScheme> liftingScheme, std::shared_ptr<const IdealGasModel> gasModel_,
                                   const NumericalFlux &rsolver, const int Np, const real_t &time, FunctionCoefficient &qn_wall,
                                   VectorFunctionCoefficient &V_wall, bool t_dependent = false);
  NoSlipAdiabWallBdrFaceIntegrator(std::shared_ptr<LiftingScheme> liftingScheme, std::shared_ptr<const IdealGasModel> gasModel_,
                                   const NumericalFlux &rsolver, const int Np, const real_t &time, real_t qn, const Vector &V);
